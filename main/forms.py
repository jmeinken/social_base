...
class ForgotPasswordForm(forms.Form):
    username_or_email = forms.CharField(max_length=50, widget=forms.TextInput, required=True, label=_('enter your username or email address'))
    ...
    def clean(self):
        ...
        if not qUser:
            self.add_error('username_or_email', _('That username or email address wasn\'t found in our system.'))
...
class CreateAccountForm(forms.ModelForm):
    password1 = forms.CharField(max_length=50, widget=forms.PasswordInput, required=True, label=_('Password'), help_text=_('Minimum 8 characters.'))
    password2 = forms.CharField(max_length=50, widget=forms.PasswordInput, required=True, label=_('Reenter Password'))
    ...
    def clean(self):
        ...
        if not pw1 and not self.editing_existing:
            self.add_error('password1', _('A password is required.'))
        if pw1:
            if len(pw1) < 8:
                self.add_error('password1', _('Password must be at least 8 characters.'))
            if pw1 != pw2:
                self.add_error('password2', _('Password values didn\'t match.'))
...
class PasswordForm(forms.ModelForm):
    password1 = forms.CharField(max_length=50, widget=forms.PasswordInput, required=False, label=_('Password'), help_text=_('Minimum 8 characters.'))
    password2 = forms.CharField(max_length=50, widget=forms.PasswordInput, required=False, label=_('Reenter Password'))
